...
def detect_model(device_name: str | None) -> DeviceModel:
    ...
    for advertised_code, model in iter_model_codes_by_specificity():
        if device_name.startswith(advertised_code):
            max_power_watts = model_power_watts_for_code(model.name, advertised_code)
            return replace(model, max_power_watts=max_power_watts) if max_power_watts is not None else model
    return FALLBACK
