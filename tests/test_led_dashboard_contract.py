...
def test_notify_debug_file_setting_is_sent_to_led_services() -> None:
    ...
    assert "schedule verification {_debug_schedule_target_range(target)}" in services
...
def test_scheduler_verification_is_queued_per_schedule_row() -> None:
    ...
    assert "targets = sorted(" in services
    assert 'key=lambda target: not any(int(value) > 0 for value in target.get("levels", {}).values()),' in services
    assert "primary_points = [" in services
    assert "if _ranges_match(primary_points, fallback_expected_levels):" in services
...
def test_scheduler_front_delete_opens_running_dialog() -> None:
    ...
    assert 'service: "remove_schedule"' in implementation
